...
const ToggleButton = ({
  ...
  const buttonClasses = classNames(styles.toggle_button, styles[`toggle_button__${size}`], [
    styles[
      `toggle_button__${size}__${darkMode ? "dark" : "light"}__${enabled ? "enabled" : "disabled"}`
    ],
  ]);

  const containerClasses = classNames(styles.toggle_container, styles[`toggle_container__${size}`]);
