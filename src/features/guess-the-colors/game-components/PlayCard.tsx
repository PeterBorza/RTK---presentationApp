...
const PlayCard = ({ attempt, gameCombo, isFinished, gameData }: PlayCardType) => {
  ...
  const checkIfIncluded = (playerCombo: IguessGameItem[]) =>
    gameCombo.filter((item, index) => playerCombo.includes(item) && playerCombo[index] !== item);
  ...
  return (
    <div className={playCardClasses} onClick={() => dispatch(selectAttempt(id))} ref={playCardRef}>
